...
type AddNewTaskModalProps = {
  toggleTodo: () => void;
  onTodoAdded: (todo: Todo) => void; // callback to parent
};

export default function AddNewTaskModal({ toggleTodo, onTodoAdded }: AddNewTaskModalProps) {
  const { createTodo } = useTodos();

  const [title, setTitle] = React.useState("");
  const [description, setDescription] = React.useState("");
  const [priority, setPriority] = React.useState<"Extreme" | "Moderate" | "Low">("Moderate");
...
const handleSubmit = async () => {
  ...
  try {
    const newTodo: Partial<Todo> = {
      title,
      description,
      priority: priority.toLowerCase() as "extreme" | "moderate" | "low",
      is_completed: false,
      todo_date: date,
    };
                ...
                <div key={p} className="flex items-center gap-2">
                  <div
                    className={`w-1.5 h-1.5 rounded-full ${
                      p === "Extreme" ? "bg-red" : p === "Moderate" ? "bg-green" : "bg-yellow-500"
                    }`}
                  ></div>
                  <p className="text-sm font-normal text-dark-gray">{p}</p>
                  <input
                    type="radio"
                    name="priority"
                    checked={priority === p}
